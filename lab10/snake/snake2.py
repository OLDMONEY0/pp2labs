...
conn = psycopg2.connect("dbname=postgres user=postgres password = MaryRay")
cur = conn.cursor()
    ...
    while menu:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and 430 <= mx <= 550 and 570 <= my <= 600:
                global user, record
                user = input()
                select = "SELECT score FROM public.snake WHERE username = '{0}';"
    ...
    if x < block or x > width - 2 * block or y < block or y > height - 2 * block or len(snake) > len(set(snake)):
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    if record != None: 
                        delete = "DELETE FROM public.snake WHERE username = '{0}';"
                        cur.execute(delete.format(user))
                        conn.commit()
                        insert = "INSERT INTO public.snake(username, score) VALUES ('{0}', {1});"
